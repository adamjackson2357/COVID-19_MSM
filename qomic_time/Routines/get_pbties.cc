                            ...
                            double lambda_24)
{
    ...
    double cancer = mat_covars.matrix[ind*mat_covars.nb_columns+12];
    double cardio = mat_covars.matrix[ind*mat_covars.nb_columns+13];
    double hyper = mat_covars.matrix[ind*mat_covars.nb_columns+14];
    double diab = mat_covars.matrix[ind*mat_covars.nb_columns+15];
    double resp = mat_covars.matrix[ind*mat_covars.nb_columns+16];
    double immuno = mat_covars.matrix[ind*mat_covars.nb_columns+17];
    double ace = mat_covars.matrix[ind*mat_covars.nb_columns+18];
    double angio = mat_covars.matrix[ind*mat_covars.nb_columns+19];
    double steroids = mat_covars.matrix[ind*mat_covars.nb_columns+20];
    double statins = mat_covars.matrix[ind*mat_covars.nb_columns+21];

    // Get the time covariate
    double hospital = mat_hospital.matrix[day*mat_hospital.nb_columns+time_col];
    ...
    double arg_tot = mu + lambda_1*age + lambda_2*sex + lambda_3*ethnic_other + lambda_4*ethnic_black +
        ...
        lambda_13*cancer + lambda_14*cardio + lambda_15*hyper + lambda_16*diab + lambda_17*resp + lambda_18*immuno +
        lambda_19*ace + lambda_20*angio + lambda_21*steroids + lambda_22*statins +
        lambda_23*hospital+lambda_24*hospital*age;
                ...
                std::vector<int> final_indicator)
{
    ...
    double lambda_13_0 = theta_temp->data[13];
    double lambda_14_0 = theta_temp->data[14];
    double lambda_15_0 = theta_temp->data[15];
    double lambda_16_0 = theta_temp->data[16];
    double lambda_17_0 = theta_temp->data[17];
    double lambda_18_0 = theta_temp->data[18];
    double lambda_19_0 = theta_temp->data[19];
    double lambda_20_0 = theta_temp->data[20];
    double lambda_21_0 = theta_temp->data[21];
    double lambda_22_0 = theta_temp->data[22];
    double lambda_23_0 = theta_temp->data[23];
    double lambda_24_0 = theta_temp->data[24];
    
    double mu_1 = theta_temp->data[25];

    for(unsigned int ind=0;ind<n_ind;ind++){
        unsigned int first_day = start_indicator[ind];
        unsigned int final_day = final_indicator[ind];
        ...
        for(unsigned int day=first_day;day<=final_day;day++){
            ...
            prob=prob*get_I_W(mat_covars,
                              ...
                              lambda_24_0);
                cum_prob-=prob;
                mat_P_I_W.matrix[ind*mat_P_I_W.nb_columns+day]=prob;

                // Get the I-R transition
                prob=cum_prob;
                prob=prob*get_intercept_prob(mu_1);
                cum_prob-=prob;
                mat_P_I_R.matrix[ind*mat_P_I_R.nb_columns+day]=prob;

                // Get the I-I transition
                prob=cum_prob;
                mat_P_I_I.matrix[ind*mat_P_I_I.nb_columns+day]=prob;
        }
    }
}
    ...
    for(unsigned int ind=0;ind<n_ind;ind++){
        ...
        for(unsigned int day=first_day;day<=final_day;day++){
            ...
            prob=prob*get_I_W(mat_covars,
                              ...
                              lambda_24_0);
                cum_prob-=prob;
                mat_P_W_D.matrix[ind*mat_P_W_D.nb_columns+day]=prob;

                // Get the W-R transition
                prob=cum_prob;
                prob=prob*get_intercept_prob(mu_1);
                cum_prob-=prob;
                mat_P_W_R.matrix[ind*mat_P_W_R.nb_columns+day]=prob;

                // Get the W-W transition
                prob=cum_prob;
                mat_P_W_W.matrix[ind*mat_P_W_W.nb_columns+day]=prob;
        }
    }
}
